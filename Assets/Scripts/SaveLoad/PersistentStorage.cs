...
namespace Voxels.SaveLoad
{
	public class PersistentStorage
	{
		...
		public void SaveGame(Vector3 playerPosition, Vector3 playerRotation)
		{
			...
			// world parameters
			_writer.Write((byte)Constants.CHUNK_SIZE);
			_writer.Write((byte)GlobalVariables.Settings.WorldSizeX);
			_writer.Write((byte)Constants.WORLD_SIZE_Y);
			_writer.Write((byte)GlobalVariables.Settings.WorldSizeZ);
			...
			for (x = 0; x < GlobalVariables.TotalBlockNumberX; x++)
				for (z = 0; z < GlobalVariables.TotalBlockNumberZ; z++)
					for (y = 0; y < GlobalVariables.TotalBlockNumberY; y++)
		...
		ChunkData ReadChunk() => new ChunkData(ReadVector3Int(), ReadVector3Int()) { Status = ChunkStatus.NeedToBeRedrawn };
